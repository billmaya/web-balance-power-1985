/*
Both the _NEW and _NEWER functions use the refactored map_data_NEW structure, which allows map_data to contain multiple non-adjacent regions for a single country.

_NEW functions draw the three North American countries. Non-adjacent regions are drawn as a single SVG image so all regions are highlighted when one region is highlighted.
This is done using Polygons so an extra line is drawn between two non-adjacent regions.

_NEWER functions also draw the three North American countries but attempt to use Paths instead of Polygons since Paths allow MoveTo and LineTo commands.
Unfortunately this does not work right now.
*/
